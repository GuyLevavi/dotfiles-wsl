...
The bundle script downloads:
- 18 CLI tool binaries (starship, zoxide, fzf, bat, eza, ripgrep, fd, yazi, lazygit, uv, glab, jf, delta, nvim, helm, oc, fastfetch, mc)
- System RPMs/DEBs for offline package install
- Zsh plugins (zinit, fast-syntax-highlighting, zsh-autosuggestions, zsh-completions, fzf-tab)
- **Neovim/LazyVim plugin cache** (auto-generated by bundle.sh)
- Dotfiles via the repo

The deploy script unpacks everything and runs the bootstrap steps offline.
...
| Image | What it contains | Use when |
|-------|------------------|----------|
| `Dockerfile.airgap` (default) | Base packages + user + **dotfiles stowed** + zsh plugins | **You want this** — fully ready, works offline |
| `Dockerfile` (base) | Only packages + user, no configs | Building custom images, CI base layers |

The **airgap-ready images** published to GHCR include:
- All system packages pre-installed
- User `gl` with sudo
- Dotfiles stowed (`.zshrc`, `.config/nvim`, etc.)
- Zsh plugins
- Neovim/LazyVim plugins (cached in the bundle)
- CLI tools must still be installed via deploy.sh or manually
...
> **IMPORTANT:** The Docker images require you to install CLI tools before use.
> Either use the bundle (`airgap/deploy.sh`) or build with the included tools.
> See "Using the Docker images" below.
...
```bash
# Clone and build with CLI tools baked in
git clone https://github.com/GuyLevavi/dotfiles-wsl.git
cd dotfiles-wsl
bash airgap/bundle.sh  # downloads tools + nvim plugins
# Then use Dockerfile.airgap with COPY of cache/
```

### Testing nvim offline

LazyVim plugins are downloaded during `bundle.sh` and included in the bundle.
After deploying, verify nvim works fully offline:

```bash
# This should succeed WITHOUT network
nvim --headless +qa
# If this passes, plugins are cached and work offline
```

If you see errors about "Failed to download" or plugin install prompts,
the bundle is missing the plugin cache. Re-run `bundle.sh` on an online
machine to regenerate `nvim-data.tar.gz`.
